...
function AskAi() {
  ...
  async function processMessageToChatGPT(chatMessage) {
    ...
    const url = `${baseUrl}/api/chat/user1/ask?message=${encodeURIComponent(chatMessage)}&conversationId=${conversationId}`;
    ...
    if (!response.ok) {
      ...
      let errMsg = 'Failed to get response from RajeshBot';
      try {
